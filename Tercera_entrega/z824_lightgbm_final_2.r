...
PARAM$experimento <- "competencia_3_v2"

PARAM$input$dataset <- "./datasets/competencia_3.csv.gz"

# los meses en los que vamos a entrenar
#  mucha magia emerger de esta eleccion
PARAM$input$future <- c(202109) # meses donde se aplica el modelo
PARAM$input$training <- c(201906, 201907, 201908, 201909, 201910, 201911, 
                          201912, 202001, 202002,202003, 202004,202006,202008,202009,202011, 
                          202012, 202101, 202102, 
                          202103, 202104, 202105,202106,202107)


semillas <- c(162143, 193183, 231677, 274831, 312199,
              349591, 383377, 420163, 458323, 497351,
              532933, 569429, 607547, 642023, 681937,
              720983, 757619, 796867, 832153, 870077)
...
for (i in 1:20) {
  ...
  PARAM$finalmodel$optim$num_iterations <- 1431
  
  PARAM$finalmodel$optim$learning_rate <- 0.021524551	
  
  PARAM$finalmodel$optim$feature_fraction <- 0.644212476
  
  PARAM$finalmodel$optim$min_data_in_leaf <- 30730
  
  PARAM$finalmodel$optim$num_leaves <- 557
